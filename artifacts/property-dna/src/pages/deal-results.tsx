...
function StatusBadge({ status }: { status: string }) {
  ...
  return <span className={cn("inline-flex text-xs font-semibold uppercase tracking-wider px-2.5 py-0.5 rounded-full", s.cls)}>{s.label}</span>;
}
    ...
    <div className="flex flex-col items-center">
      <div className="relative w-36 h-36">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-3xl font-black text-foreground">{score.toFixed(0)}</span>
...
function MetricCard({ label, value, sub, positive }: { label: string; value: string; sub?: string; positive?: boolean }) {
  return (
    <div className="bg-card border border-border rounded-xl px-4 py-4">
      ...
      <div className={cn("text-lg font-bold", positive === true ? "text-emerald-600" : positive === false ? "text-red-600" : "text-foreground")}>{value}</div>
...
export default function DealResultsPage() {
  ...
  if (dealLoading || resultsLoading) {
    return (
      <Layout title="Deal Results">
        <div className="flex items-center justify-center h-64 text-sm text-muted-foreground">Loading deal analysis...</div>
      </Layout>
    );
  }
  if (!deal || !results) {
  ...
  const isRed = results.dealRating === "Red";
    ...
    <Layout title="Deal Analysis">
      <div className="px-8 py-7 max-w-5xl mx-auto">
        ...
        <div className={cn(
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              {isYellow && <AlertTriangle className="w-7 h-7 text-amber-500" />}
              {isRed && <XCircle className="w-7 h-7 text-red-500" />}
            </div>
          ...
          <div className="text-right">
            ...
            <div className={cn("text-3xl font-black", cashFlowPositive ? "text-emerald-700" : "text-red-700")}>
        ...
        <div className="grid grid-cols-4 gap-3 mb-6">
          ...
          <MetricCard label="Total Monthly Expenses" value={formatCurrency(results.totalMonthlyExpenses)} />
        </div>
        ...
        <div className="bg-card border border-border rounded-xl p-5 mb-6">
          ...
          <div className="grid grid-cols-3 gap-x-6">
            {[
              ...
              { label: "Total Cash Investment", value: formatCurrency(results.totalInvestment) },
        ...
        {/* AI Deal Coach placeholder */}
        {/* FUTURE: OpenAI AI Deal Coach — send deal + results JSON to GPT-4o, stream natural language analysis back */}
        <div className="bg-card border border-border rounded-xl p-5 mb-4">
          ...
          <p className="text-xs text-muted-foreground mb-3">
            GPT-4 powered analysis will review your deal numbers and provide personalized recommendations, risk flags, and negotiation tips.
          </p>
          <div className="bg-muted/50 rounded-lg px-4 py-3 border border-border">
            <p className="text-xs text-muted-foreground/70 italic">
              "This deal shows a DSCR of {formatNumber(results.dscr)}, which means the income {results.dscr >= 1 ? "covers" : "doesn't fully cover"} debt service. {results.dscr < 1.25 ? "Consider negotiating the purchase price down or finding ways to increase rental income before proceeding..." : "Strong debt coverage — this deal has good income support relative to debt obligations."}"
            </p>
          </div>
        ...
        {/* FUTURE: Stripe — createCheckoutSession on backend, redirect to Stripe hosted checkout ($49), webhook to mark deal as PAID */}
        <div className="bg-card border border-border rounded-xl p-5 mb-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
          ...
          <p className="text-xs text-muted-foreground mb-3">
            Unlock a professionally formatted PDF report with executive summary, detailed comps, risk analysis, and lender-ready documentation.
          </p>
          <div className="flex items-center gap-3">
        ...
        {deal.notes && (
          <div className="mt-4 bg-muted/50 border border-border rounded-xl px-5 py-4">
